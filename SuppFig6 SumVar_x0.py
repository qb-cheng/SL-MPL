# -*- coding: utf-8 -*-
"""
Estimator mean and variance with different values of initial mutant allele frequencies,
...
fig,axes = plt.subplots(len(Sets),3,figsize=(15,9),dpi=dpi_val)
for set_idx,thisSet in enumerate(Sets):
    ...
    cur_ax.text(0.05,0.91,'$x(0) = $' + str(x0),fontsize=FontSize,ha='left',va='center',transform=cur_ax.transAxes)
    ...
    cur_ax.tick_params(axis='y', labelsize=FontSize)
    cur_ax.xaxis.set_major_locator(ticker.MultipleLocator(150))
    cur_ax.text(0.05,0.09,'$x(0) = $' + str(x0),fontsize=FontSize,ha='left',va='center',transform=cur_ax.transAxes)
